...
return (      <section className="assistant-result">
        <p className="mb-5 border-b pb-3 text-sm text-muted-foreground">{result.cache?.hit?t('저장된 분석 결과 · 10분 이내','Cached analysis · within 10 minutes'):<>{t('분석 완료','Analysis complete')} · {((result.timing?.totalMs || 0)/1000).toFixed(1)}s</>}</p><div className="mb-6"><p className="text-sm text-muted-foreground">{result.video.channel}</p><h2 className="mt-1 text-xl font-semibold">{result.video.title}</h2></div>
