        ...
        {author ? <>
          ...
          {comments.slice(0,limit).map((c:any,i:number)=>{
            ...
            return <article key={c.id} className="thread-record">
              {relation&&<div className="mb-2 text-xs text-muted-foreground">{relation.status==='content'?t('이 댓글에 대한 답변으로 추정 · 내용 비교','Likely a reply to this comment · text similarity'):relation.status==='missing'?t('태그한 상대의 댓글이 수집 결과에 없습니다.','The tagged participant’s comment is not in the collected results.'):t('내용만으로 답변 대상을 하나로 특정하지 못했습니다.','The text does not identify one clear reply target.')}
                {!!relation.candidates.length&&<details className="mt-2"><summary>{t('다른 후보 보기','Other possible targets')} ({relation.candidates.length})</summary>{relation.candidates.map((candidate:any)=><div key={candidate.id} className="mt-2 rounded-xl bg-muted p-3"><strong>{candidate.author}</strong> · {commentTime(candidate)}<p className="mt-1 whitespace-pre-wrap">{candidate.text}</p></div>)}</details>}
              </div>}
              {relation&&rootParent&&rootParent.id!==parent?.id&&<details className="mb-3 text-xs text-muted-foreground"><summary>{t('최상위 댓글 보기','View thread root')}</summary><div className="mt-2 rounded-xl bg-muted p-3"><strong>{rootParent.author}</strong><p className="mt-1 whitespace-pre-wrap">{rootParent.text}</p></div></details>}
