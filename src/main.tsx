...
return (      <section className="assistant-result">
        ...
        <section className="summary-accordions" aria-label={t('댓글 내용 요약','Comment content summary')}>
          <details className="insight-panel"><summary className="insight-heading"><span>{t('많이 나타난 의견','Most common viewpoints')}</span><ChevronDown className="size-4 shrink-0"/></summary><div className="accordion-content"><p className="mb-4 text-xs text-muted-foreground">{t('비슷한 댓글의 공통 주제와 반응을 요약했습니다. 의견을 누르면 판단 근거가 된 실제 댓글을 볼 수 있습니다.','We summarized shared themes and reactions. Select a viewpoint to inspect the original comments behind it.')}</p>{!result.opinions?.length&&<p className="text-sm text-muted-foreground">{t('반복되는 의견을 충분히 찾지 못했습니다.','No sufficiently repeated viewpoints were found.')}</p>}<ol className="opinion-list">{(result.opinions||[]).map((item:any,index:number)=><li key={item.commentIds[0]}><button type="button" aria-expanded={opinion?.commentIds[0]===item.commentIds[0]} onClick={()=>setOpinion(opinion?.commentIds[0]===item.commentIds[0]?null:item)}><span className="opinion-rank">{index+1}</span><p>{item.summary||item.representative}</p><Badge variant="secondary" className="shrink-0 rounded-full">{number(item.count)}{t('개','')}</Badge></button>{opinion?.commentIds[0]===item.commentIds[0]&&<div className="opinion-comments">{item.commentIds.map((id:string)=>commentsById.get(id)).filter(Boolean).map((c:any)=><article key={c.id}><div className="flex flex-wrap items-center justify-between gap-2"><strong>{c.author}</strong><span>{c.parentId?t('대댓글','Reply'):t('댓글','Comment')}{commentTime(c)?` · ${commentTime(c)}`:''} · {t('좋아요','Likes')} {number(c.likeCount)}</span></div><p>{c.text}</p></article>)}</div>}</li>)}</ol></div></details>
