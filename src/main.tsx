...
function App() {
  ...
  async function analyze(e:React.FormEvent){
    ...
    const submittedUrl=url.trim();setBusy(true);setError('');setProgress({stage:'video_context'});setLines([]);setUrl('');
    ...
    const seen=new Set();
    const onProgress=(p:any)=>{setProgress((old:any)=>({...old,...p,stage:p.stage==='connecting'?'video_context':p.stage}));if(p.videoContextStatus)setTurns(old=>old.map(turn=>turn.id===jobId?{...turn,videoContextStatus:p.videoContextStatus}:turn));const fresh=(p.recent||[]).filter((c:any)=>{if(seen.has(c.id))return false;seen.add(c.id);return true;});if(fresh.length)setLines(old=>[...old,...fresh].slice(-40));};
    try{
  ...
  return <div className="min-h-svh">
    ...
    <main className={`mx-auto max-w-5xl px-5 sm:px-8 ${turns.length?'chat-main':''}`}>
      ...
      <div className="chat-history">{turns.map((turn,index)=><div id={`turn-${turn.id}`} key={turn.id} ref={index===turns.length-1?latestRef:null} className="chat-turn"><div className="mb-10 flex flex-col items-end"><div className="max-w-[85%] rounded-3xl bg-muted px-5 py-4"><a href={turn.url} target="_blank" rel="noreferrer" className="break-all text-sm underline-offset-4 hover:underline">{turn.url}</a><p className="mt-2 text-sm">{t('이 영상의 댓글을 분석해주세요.','Analyze the comments on this video.')}</p></div><p className="mt-2 pr-2 text-xs text-muted-foreground">{new Date(turn.time).toLocaleTimeString(language==='ko'?'ko-KR':'en-US',{hour:'2-digit',minute:'2-digit'})}</p></div>{turn.videoContextStatus&&<p className="mx-auto mb-4 max-w-4xl text-xs text-muted-foreground">{turn.videoContextStatus==='available'?t('영상 내용 확인 완료 · 댓글 분석에 반영','Video content checked · Used for comment analysis'):turn.videoContextStatus==='partial'?t('영상 내용 일부 확인 · 확보한 내용을 댓글 분석에 반영','Partial video content · Available text used for comment analysis'):t('영상 자막을 확보하지 못해 댓글과 답글 맥락으로 분석합니다.','Video captions unavailable · Using comments and thread context.')}</p>}{turn.result&&<ResultReply result={turn.result} language={language}/>} {turn.error&&<Alert><Info/><AlertTitle>{t('분석하지 못했습니다','Analysis failed')}</AlertTitle><AlertDescription>{turn.error}</AlertDescription></Alert>}      {!turn.result&&!turn.error&&busy&&<section className="mx-auto mb-12 max-w-4xl"><div role="status" className="mb-4 flex justify-between gap-4 text-sm"><span>{progress.stage==='video_context'?t('영상 내용 분석 중','Analyzing video content'):progress.stage==='classifying'||progress.stage==='finalizing'?t('댓글 분석 중','Analyzing comments'):t('공개 댓글 수집 중','Collecting public comments')}{progress.stage!=='video_context'&&<> · {number(progress.done)}{progress.total?` / ${number(progress.total)}`:''}</>}</span><span className="inline-flex shrink-0 items-center gap-2"><LoaderCircle aria-hidden="true" className="size-4 animate-spin"/>{progress.stage==='classifying'&&progress.total?`${Math.floor(progress.done/progress.total*100)}%`:t('진행 중','In progress')}</span></div><div ref={streamRef} className="stream h-48 overflow-auto rounded-lg bg-muted p-3 text-xs leading-6">{lines.length?lines.map(c=><div key={c.id} className="truncate"><b>{c.author}</b>　{c.text}</div>):progress.stage==='video_context'?t('영상 내용을 확인하고 있습니다…','Reading video content…'):t('YouTube에 연결하고 있습니다…','Connecting to YouTube…')}</div></section>}
</div>)}</div>
    </main>
    </div>
...
return (      <section className="assistant-result">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3 border-b pb-3"><p className="text-sm text-muted-foreground">{result.cache?.hit?t('저장된 분석 결과 · 10분 이내','Cached analysis · within 10 minutes'):<>{t('분석 완료','Analysis complete')} · {((result.timing?.totalMs || 0)/1000).toFixed(1)}s</>}</p><Button variant="outline" onClick={exportCsv}><Download/>{t('CSV 내보내기','Export CSV')}</Button></div><div className="mb-6"><p className="text-sm text-muted-foreground">{result.video.channel}</p><h2 className="mt-1 text-xl font-semibold">{result.video.title}</h2></div>
        ...
        <section className="summary-accordions" aria-label={t('댓글 내용 요약','Comment content summary')}>
          <details className="insight-panel"><summary className="insight-heading"><span>{t('많이 언급된 주제','Most discussed topics')}</span><ChevronDown className="size-4 shrink-0"/></summary><div className="accordion-content"><p className="mb-4 text-xs text-muted-foreground">{t('같은 주제의 댓글을 하나로 묶었습니다. 찬반과 선플·악플 판정은 댓글마다 다를 수 있습니다. 주제를 누르면 원문을 볼 수 있습니다.','Comments on the same topic are grouped together. Individual viewpoints and sentiment labels may differ. Select a topic to read the comments.')}</p>{!result.opinions?.length&&<p className="text-sm text-muted-foreground">{t('반복되는 의견을 충분히 찾지 못했습니다.','No sufficiently repeated viewpoints were found.')}</p>}<ol className="opinion-list">{(result.opinions||[]).map((item:any,index:number)=><li key={item.commentIds[0]}><button type="button" aria-expanded={opinion?.commentIds[0]===item.commentIds[0]} onClick={()=>setOpinion(opinion?.commentIds[0]===item.commentIds[0]?null:item)}><span className="opinion-rank">{index+1}</span><p>{item.summary||item.representative}</p><Badge variant="secondary" className="shrink-0 rounded-full">{number(item.count)}{t('개','')}</Badge></button>{opinion?.commentIds[0]===item.commentIds[0]&&<div className="opinion-comments">{item.commentIds.map((id:string)=>commentsById.get(id)).filter(Boolean).map((c:any)=><article key={c.id}><div className="flex flex-wrap items-center justify-between gap-2"><strong>{c.author}</strong><span>{c.parentId?t('대댓글','Reply'):t('댓글','Comment')}{commentTime(c)?` · ${commentTime(c)}`:''} · {t('좋아요','Likes')} {number(c.likeCount)}</span></div><p>{c.text}</p><span className="text-xs text-muted-foreground">{({positive:t('선플','Kind'),negative:t('악플','Abusive'),neutral:t('중립','Neutral'),mixed:t('혼합','Mixed')} as Record<string,string>)[c.sentiment?.label]||''}</span></article>)}</div>}</li>)}</ol></div></details>
          <details className="insight-panel"><summary className="insight-heading"><span>{t('자주 등장한 단어','Frequent terms')}</span><ChevronDown className="size-4 shrink-0"/></summary><div className="accordion-content"><div className="flex flex-wrap gap-2">{[...result.topics].sort((a:any,b:any)=>b.count-a.count).map((v:any)=><Badge variant="secondary" className="rounded-full px-3 py-1 text-xs font-normal" key={v.term}>{v.term}<span className="ml-1 text-blue-600 tabular-nums">{number(v.count)}</span></Badge>)}</div></div></details>
        </section>
