...
function App() {
  ...
  async function analyze(e:React.FormEvent){
    ...
    const jobId=crypto.randomUUID();setTurns(old=>[...old,{id:jobId,url:submittedUrl,analysisLanguage,time:Date.now()}]);setTimeout(()=>latestRef.current?.scrollIntoView({block:'start',behavior:'smooth'}),60); let stopped=false;let pending=false;
    const seen=new Set();
    const poll=async()=>{if(pending||stopped)return;pending=true;try{const r=await fetch(`/api/progress?id=${jobId}`);if(!r.ok)return;const p=await r.json();if(stopped)return;setProgress(p);const fresh=(p.recent||[]).filter((c:any)=>{if(seen.has(c.id))return false;seen.add(c.id);return true;});if(fresh.length)setLines(old=>[...old,...fresh].slice(-40));}catch{}finally{pending=false;}};
    const timer=setInterval(poll,500);
    try{const response=await fetch('/api/analyze',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url:submittedUrl,jobId,analysisLanguage})});const data=await response.json();if(!response.ok)throw new Error(data.error||t('분석하지 못했습니다.','Analysis failed.'));setTurns(old=>old.map(turn=>turn.id===jobId?{...turn,result:data}:turn));}
    catch(e:any){setTurns(old=>old.map(turn=>turn.id===jobId?{...turn,error:e.message}:turn));}finally{stopped=true;clearInterval(timer);setBusy(false);}
  }
...
return (      <section className="assistant-result">
        ...
        {result.truncated&&<Alert className="mb-6"><Info/><AlertTitle>{t('일부 댓글만 수집되었습니다','Collection is incomplete')}</AlertTitle><AlertDescription>{t('수집 한도에 도달했습니다. 아래 통계는 수집된 댓글 기준입니다.','The collection limit was reached. Statistics cover collected comments only.')}</AlertDescription></Alert>}
