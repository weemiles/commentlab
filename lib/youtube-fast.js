...
export async function fetchFastVideoAndComments({ videoId, maxComments = 3000, sortBy = "new", fetchImpl = fetch, requestDelayMs = Number(process.env.YOUTUBE_REQUEST_DELAY_MS ?? 0), concurrency = Number(process.env.YOUTUBE_FETCH_CONCURRENCY ?? 8), onProgress = () => {} }) {
  const pageResponse = await fetchYouTubeWithRetry(fetchImpl, `${VIDEO_URL}${videoId}`, { headers: { "user-agent": USER_AGENT, cookie: "CONSENT=YES+cb" } });
  ...
  return {
    video: await resolveVideoMetadata(player, initialData, videoId, comments.length, fetchImpl),
    comments: comments.slice(0, maxComments),
    truncated: comments.length >= maxComments,
