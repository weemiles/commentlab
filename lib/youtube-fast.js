...
export async function fetchFastVideoAndComments({ videoId, maxComments = 3000, sortBy = "new", fetchImpl = fetch, collectorUrl = "", requestDelayMs = Number(process.env.YOUTUBE_REQUEST_DELAY_MS ?? 0), concurrency = Number(process.env.YOUTUBE_FETCH_CONCURRENCY ?? 8), onProgress = () => {} }) {
  ...
  const loadPages = async (endpoints) => {
    if (!remote) return Promise.all(endpoints.map(loadPage));
    const client = config.INNERTUBE_CONTEXT.client;
    let pages;
    try {
      ...
      return Promise.all(endpoints.map(loadPage));
    }
  ...
  while (pages.length && comments.length < maxComments) {
    ...
    const batchSize = remote ? Math.min(4, Math.max(1, concurrency)) : Math.max(1, concurrency);
