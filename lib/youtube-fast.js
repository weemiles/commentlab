...
export async function fetchFastVideoAndComments({ videoId, language = "ko", maxComments = 3000, sortBy = "new", fetchImpl = fetch, collectorUrl = "", requestDelayMs = Number(process.env.YOUTUBE_REQUEST_DELAY_MS ?? 0), concurrency = Number(process.env.YOUTUBE_FETCH_CONCURRENCY ?? 16), onProgress = () => {} }) {
  ...
  consume([firstPage]);
  const limit = Math.max(1, Math.min(16, Number(concurrency) || 16));
  if (remote) {
