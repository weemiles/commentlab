...
async function requestContinuation(endpoint, config, fetchImpl) {
  ...
  const url = new URL(apiPath, "https://www.youtube.com");
...
function findInitialContinuation(initialData) {
  const itemSection = walk(initialData, "itemSectionRenderer")[0];
...
function metadataFromPlayer(player, videoId, commentsLength) {
  ...
  return {
    ...
    channel: details.author || "채널 정보 없음",
...
export async function fetchFastVideoAndComments({ videoId, maxComments = 3000, sortBy = "new", fetchImpl = fetch, requestDelayMs = Number(process.env.YOUTUBE_REQUEST_DELAY_MS ?? 0), concurrency = Number(process.env.YOUTUBE_FETCH_CONCURRENCY ?? 8), onProgress = () => {} }) {
  const pageResponse = await fetchImpl(`${VIDEO_URL}${videoId}`, { headers: { "user-agent": USER_AGENT, cookie: "CONSENT=YES+cb" } });
  if (!pageResponse.ok) throw new Error(`YouTube 영상 페이지 요청 실패 (${pageResponse.status})`);
  const html = await pageResponse.text();
  const config = extractBalancedJson(html, ["ytcfg.set({"]);
  const initialData = extractBalancedJson(html, ["var ytInitialData =", "window[\"ytInitialData\"] =", "ytInitialData ="]);
  const player = extractBalancedJson(html, ["var ytInitialPlayerResponse =", "ytInitialPlayerResponse ="]);
  if (!config?.INNERTUBE_API_KEY || !config?.INNERTUBE_CONTEXT || !initialData) throw new Error("YouTube 내부 댓글 설정을 찾지 못했습니다.");

  let firstEndpoint = findInitialContinuation(initialData);
  if (!firstEndpoint) throw new Error("댓글이 비활성화됐거나 공개 댓글이 없습니다.");

  let firstPage = await requestContinuation(firstEndpoint, config, fetchImpl);
  const selectedSort = sortEndpoint(initialData, sortBy) || sortEndpoint(firstPage, sortBy);
  if (selectedSort) firstPage = await requestContinuation(selectedSort, config, fetchImpl);
  ...
  while (pages.length && comments.length < maxComments) {
    ...
    pages = await Promise.all(batch.map((endpoint) => requestContinuation(endpoint, config, fetchImpl)));
  }

  return {
    video: metadataFromPlayer(player, videoId, comments.length),
    comments: comments.slice(0, maxComments),
    truncated: comments.length >= maxComments,
    collectionMethod: "youtube-fast-continuation"
  };
}
