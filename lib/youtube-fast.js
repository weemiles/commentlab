...
export function extractBalancedJson(source, markers) {
...
async function requestInnertube(apiPath, payload, config, fetchImpl) {
  if (!["/youtubei/v1/next", "/youtubei/v1/browse"].includes(apiPath)) throw localeError("unsupported_collect_path", { status: 400 });
  ...
  if (!response.ok) throw localeError("youtube_request_failed", { status: response.status, params: { status: response.status } });
...
async function loadVideoSession(videoId, fetchImpl, language = "ko") {
  const hl = language === "en" ? "en" : "ko";
  const gl = hl === "en" ? "US" : "KR";
  let watchError;
  try {
    const response = await fetchYouTubeWithRetry(fetchImpl, `${VIDEO_URL}${videoId}&hl=${hl}&gl=${gl}`, {
      headers: { "user-agent": USER_AGENT, "accept-language": hl, cookie: "CONSENT=YES+cb" },
      timeoutMs: 10000
    });
    if (!response.ok) throw localeError("youtube_request_failed", { status: response.status, params: { status: response.status } });
    ...
    if (!config?.INNERTUBE_CONTEXT?.client || !initialData) throw localeError("youtube_unavailable");
    config.INNERTUBE_CONTEXT.client.hl = hl;
    config.INNERTUBE_CONTEXT.client.gl = gl;
  ...
  const config = {
    ...
    INNERTUBE_CONTEXT: { client: { clientName: "WEB", clientVersion: WEB_CLIENT_VERSION, hl, gl } }
  };
  try {
    const initialData = await requestInnertube("/youtubei/v1/next", { videoId }, config, fetchImpl);
    return { config, initialData, player: null };
  } catch (error) {
    console.warn(`Watch page and web client both failed: ${watchError.message}; ${error.message}`);
    throw localeError(error.code === "youtube_request_failed" ? "youtube_request_failed" : "youtube_unavailable", { status: error.status, params: error.params, cause: error });
  }
}
...
export async function fetchCollectorSession(videoId, fetchImpl = fetch, language = "ko") {
  const { config, initialData, player } = await loadVideoSession(videoId, fetchImpl, language);
  const firstEndpoint = findInitialContinuation(initialData);
  if (!firstEndpoint) throw localeError("comments_unavailable", { status: 404 });
  const client = config.INNERTUBE_CONTEXT.client;
  return {
    video: await resolveVideoMetadata(player, initialData, videoId, 0, fetchImpl, language),
...
export async function requestCollector(collectorUrl, body, fetchImpl) {
  const response = await fetchImpl(collectorUrl, {
    method: "POST", headers: { "content-type": "application/json", ...(process.env.YOUTUBE_COLLECTOR_TOKEN ? { authorization: `Bearer ${process.env.YOUTUBE_COLLECTOR_TOKEN}` } : {}) },
    body: JSON.stringify(body), signal: AbortSignal.timeout(45000)
  });
  if (!response.ok) {
    // The collector's own error text is upstream data; log it, never return it.
    const detail = await response.json().catch(() => ({}));
    console.warn(`Collector responded ${response.status}: ${detail.error || "no detail"}`);
    throw localeError("collector_unavailable", { status: response.status });
  }
...
export async function resolveVideoMetadata(player, initialData, videoId, commentsLength, fetchImpl = fetch, language = "ko") {
  const video = metadataFromSources(player, initialData, videoId, commentsLength);
  {
    try {
      ...
      url.searchParams.set("hl", language === "en" ? "en" : "ko");
      const response = await fetchImpl(url, { headers: { "accept-language": language === "en" ? "en" : "ko" }, signal: AbortSignal.timeout(5000) });
      if (response.ok) {
        const embed = await response.json();
        video.title = embed.title || video.title;
...
export async function fetchChannelVideoIds({ channelId, maxVideos = 10, fetchImpl = fetch }) {
  if (!channelId) throw localeError("channel_not_found", { status: 400 });
  ...
  const config = extractBalancedJson(html, ["ytcfg.set({"]);
  const initialData = extractBalancedJson(html, ["var ytInitialData =", "window[\"ytInitialData\"] =", "ytInitialData ="]);
  if (!config?.INNERTUBE_API_KEY || !config?.INNERTUBE_CONTEXT || !initialData) throw localeError("youtube_unavailable");
  const ids = [];
  const seen = new Set();
...
export async function fetchFastVideoAndComments({ videoId, language = "ko", maxComments = 3000, sortBy = "new", fetchImpl = fetch, collectorUrl = "", requestDelayMs = Number(process.env.YOUTUBE_REQUEST_DELAY_MS ?? 0), concurrency = Number(process.env.YOUTUBE_FETCH_CONCURRENCY ?? 32), onProgress = () => {} }) {
  const remote = collectorUrl ? await requestCollector(collectorUrl, { action: "session", videoId, language }, fetchImpl) : null;
  let { config, initialData, player } = remote
    ? { config: { INNERTUBE_CONTEXT: { client: remote.client } }, initialData: {}, player: null }
    : await loadVideoSession(videoId, fetchImpl, language);
  ...
  const loadPages = async (endpoints) => {
    if (!remote) {
      ...
      for (const result of settled) {
        if (result.status === "fulfilled" && result.value) pages.push(result.value);
          ...
          failedPages += 1;
          console.warn(`Failed YouTube continuation: ${result.reason?.message || result.reason}`);
        }
      }
      return pages;
    }
    ...
    try {
      ...
      const settled = await Promise.allSettled(endpoints.map(loadPage));
      failedPages += settled.filter((result) => result.status === 'rejected').length;
  ...
  const initialPage = async (endpoint) => {
    const knownSort = remote ? endpointFor(sortBy === "top" ? remote.topToken : remote.newestToken) : sortEndpoint(initialData, sortBy);
    // A session can already contain the selected sorting token; avoid fetching
    // and discarding the default page just to retrieve that same token.
    if (knownSort) return loadPage(knownSort);
    let page = await loadPage(endpoint);
    const selectedSort = sortEndpoint(page, sortBy);
    if (selectedSort) page = await loadPage(selectedSort);
    return page;
  };
  let firstPage;
  try {
    firstPage = await initialPage(firstEndpoint);
  } catch (error) {
    if (remote || ![403, 429].includes(error.status)) throw error;
    // Continuation tokens belong to their visitor session. Refresh both together.
    await sleep(1500);
    ({ config, initialData, player } = await loadVideoSession(videoId, fetchImpl));
    const refreshedEndpoint = findInitialContinuation(initialData);
    if (!refreshedEndpoint) throw error;
    try { firstPage = await initialPage(refreshedEndpoint); }
    catch (retryError) {
      if (![403, 429].includes(retryError.status)) throw retryError;
      throw localeError("youtube_rate_limited", { status: retryError.status, cause: retryError });
    }
  }
  ...
  const comments = [];
  const enqueue = (endpoint, priority = false) => {
    const token = continuationToken(endpoint);
    if (!token || seenTokens.has(token)) return;
    seenTokens.add(token);
    if (priority) queue.unshift(endpoint);
    else queue.push(endpoint);
  };
  ...
  const consume = (pages) => {
    ...
    for (const page of pages) {
      ...
      // YouTube's wording is upstream data; keep it in the log, not the response.
      if (externalError) {
        console.warn(`YouTube returned an external error: ${externalError}`);
        throw localeError("youtube_rejected");
      }
      ...
      for (const action of actions) {
        for (const item of action.continuationItems || []) {
          if (action.targetId === "comments-section" || action.targetId === "engagement-panel-comments-section" || action.targetId === "shorts-engagement-panel-comments-section" || String(action.targetId).startsWith("comment-replies-item")) {
            for (const endpoint of walk(item, "continuationEndpoint")) enqueue(endpoint, !String(action.targetId).startsWith("comment-replies-item") && Boolean(item.continuationItemRenderer));
  ...
  consume([firstPage]);
  const limit = Math.max(1, Math.min(32, Number(concurrency) || 32));
  if (remote) {
    while (queue.length && comments.length < maxComments) {
      if (requestDelayMs > 0) await sleep(requestDelayMs);
      consume(await loadPages(queue.splice(0, limit)));
    }
  } else {
    // Refill a free slot immediately: a slow reply must not stall every thread.
    ...
    const launch = endpoint => {
      const task = (async () => {
        ...
        try {
          if (requestDelayMs > 0) await sleep(requestDelayMs);
          page = await loadPage(endpoint);
        } catch {
          failedPages += 1;
          return;
        }
        try {
          if (!fatal && comments.length < maxComments) consume([page]);
        } catch (error) { fatal = error; }
      })().finally(() => active.delete(task));
      active.add(task);
    };
    while (!fatal && (queue.length || active.size) && comments.length < maxComments) {
      while (queue.length && active.size < limit && comments.length < maxComments) launch(queue.shift());
  ...
  return {
    video: remote ? { ...remote.video, commentCount: comments.length } : await resolveVideoMetadata(player, initialData, videoId, comments.length, fetchImpl, language),
    comments: comments.slice(0, maxComments),
    truncated: comments.length >= maxComments || failedPages > 0,
    failedPages,
