...
function topTerms(comments, limit = 50, normalizeTopics = false) {
  ...
  for (const comment of comments) {
    for (const term of (normalizeTopics ? topicTokens(comment.text) : tokenize(contentText(comment.text)))) {
...
// Normalize common Korean particles only for topic grouping, not sentiment.
function topicTokens(text) {
  return tokenize(contentText(text)).map(term => {
    if (!/^[가-힣0-9]+$/.test(term)) return term;
    const quantity = term.match(/^(\d+(?:억|만|천|조|년|개월|월|일|원|명|회))(?:이라는데|이라는|이라니|이라면|이면|인데|에|을|를|은|는|이|가|도)?$/);
    if (quantity) return quantity[1];
    const stem = term.replace(/(?:으로는|에서는|적으로|에게는|이라면|이면|인데|이니|으로|에서|에는|은|는|이|가|을|를|의|도|과|와|에)$/, '');
    return stem.length >= 2 ? stem : term;
  });
}

function opinionFingerprint(text) {
  return ngrams(topicTokens(text).join(' '), 2);
}

function opinionPolarity(text) {
  const value = contentText(text).toLowerCase();
  const support = /찬성|지지|동의|찬동|승인|수용|\b(?:support|approve|agree|accept|favor)\b/.test(value);
  const oppose = /반대|거부|거절|반박|불허|폐지|금지|\b(?:oppose|reject|disagree|against|ban)\b/.test(value);
  const negated = /(?:안|못)\s|않|아니|없|\b(?:not|never|without|don't|doesn't|isn't)\b/.test(value);
  // A negated stance, quotation or multiple stances requires interpretation.
  // Keep it separate from paraphrases rather than guessing its direction.
  if ((support && oppose) || ((support || oppose) && negated) || /["“”「」]/.test(value)) {
    return `uncertain:${normalizedText(value)}`;
  }
  if (support) return 'support';
  if (oppose) return 'oppose';
  return negated ? 'negated' : 'unspecified';
}

function summarizeOpinion(cluster, commentsById) {
  const members = cluster.commentIds.map((id) => commentsById.get(id)).filter(Boolean);
  const commonTerms = topTerms(members, 5, true).filter(({ count, term }) => count >= 2 && term !== "url");
  const terms = (commonTerms.length ? commonTerms : topTerms(members, 2, true)).slice(0, 2).map(({ term }) => term);
  ...
  if (english) return `Comments about ${terms.join(" and ") || "this topic"}`;
  return `${subject} 관련 댓글`;
}

function opinionGroups(comments, limit = 30) {
  const clusters = [];
  const candidates = comments
    .filter((comment) => normalizedText(contentText(comment.text)).length >= 12)
    .sort((a, b) => (b.likeCount || 0) - (a.likeCount || 0));
  for (const comment of candidates) {
    const fingerprint = opinionFingerprint(comment.text);
    const label = comment.sentiment?.label || "neutral";
    const negated = opinionPolarity(comment.text);
    let match = null;
    let best = 0;
    for (const cluster of clusters) {
      if (cluster.label !== label || cluster.negated !== negated) continue;
    ...
    if (match && best >= 0.25) {
      ...
      clusters.push({ representative: comment.text, fingerprint, label, negated, count: 1, likes: comment.likeCount || 0, commentIds: [comment.id] });
    }
  }
  const commentsById = new Map(comments.map((comment) => [comment.id, comment]));
  // Merge overlapping topic groups before ranking/limiting. Recompute topics
  // from all evidence after each merge, and count each comment exactly once.
  const grouped = [];
  const topicCache = new Map();
  const termsFor = item => {
    if (!topicCache.has(item)) topicCache.set(item, new Set(topTerms(item.commentIds.map(id => commentsById.get(id)), 5, true).map(item => item.term)));
    return topicCache.get(item);
  };
  for (const cluster of clusters.sort((a, b) => b.count - a.count)) {
    const terms = termsFor(cluster);
    const match = grouped.find(other => {
      if (other.label !== cluster.label || other.negated !== cluster.negated) return false;
      const otherTerms = termsFor(other);
      const shared = [...terms].filter(term => otherTerms.has(term)).length;
      return shared >= 2 && shared / Math.min(terms.size, otherTerms.size) >= 0.5;
    });
    if (match) {
      match.commentIds = [...new Set([...match.commentIds, ...cluster.commentIds])];
      topicCache.delete(match);
      match.count = match.commentIds.length;
      match.likes = match.commentIds.reduce((sum, id) => sum + (commentsById.get(id)?.likeCount || 0), 0);
    } else grouped.push(cluster);
  }
  // The visible list groups TOPICS, not attitudes. Keep distinct viewpoints
  // nested so opposing claims are never presented as one shared opinion.
  const topics = [];
  for (const group of grouped.sort((a, b) => b.count - a.count)) {
    const terms = termsFor(group);
    const match = topics.find(topic => {
      const otherTerms = termsFor(topic);
      const shared = [...terms].filter(term => otherTerms.has(term)).length;
      const sharedQuantities = [...terms].filter(term => /^\d/.test(term) && otherTerms.has(term));
      if (sharedQuantities.length >= 2) return true;
      const anchors = [...terms].slice(0, 2), otherAnchors = [...otherTerms].slice(0, 2);
      return (anchors.length === 2 && anchors.every(term => otherAnchors.includes(term))) ||
        (shared >= 2 && shared / Math.min(terms.size, otherTerms.size) >= 0.5);
    });
    const viewpoint = { stance: group.negated, sentiment: group.label, commentIds: [...group.commentIds] };
    if (match) {
      match.commentIds = [...new Set([...match.commentIds, ...group.commentIds])];
      match.count = match.commentIds.length;
      match.likes = match.commentIds.reduce((sum, id) => sum + (commentsById.get(id)?.likeCount || 0), 0);
      match.viewpoints.push(viewpoint);
      topicCache.delete(match);
    } else topics.push({ ...group, viewpoints: [viewpoint] });
  }
  return topics
    .filter((cluster) => cluster.count >= 2)
    .sort((a, b) => b.count - a.count || b.likes - a.likes)
    .slice(0, limit)
    .map(({ fingerprint, label, negated, ...cluster }) => ({ ...cluster, summary: summarizeOpinion(cluster, commentsById) }));
}
...
export function analyzeComments(comments, sentiments = null) {
  ...
  return {
    ...
    opinions: opinionGroups(analyzed.filter((comment) => !comment.suspicious.label)),
