...
async function config() {
  ...
  const candidates = [process.env.JEV_ENV_PATH].filter(Boolean);
  let fileEnv = {};
  ...
  if (!apiKey) { const error = new Error('Jev TypeSafe API key is not configured.'); error.code = 'JEV_NOT_CONFIGURED'; throw error; }
...
async function classifyBatch(batch, offset, jevConfig, language, video, fetchImpl) {
  ...
  if (!response.ok) { const error = new Error(`Jev API가 HTTP ${response.status}를 반환했습니다.`); error.status = response.status; throw error; }
...
export async function classifySentimentWithJev(comments, { language = "ko", batchSize = 20, concurrency = 4, video = {}, fetchImpl = fetch, apiKey, onProgress = () => {} } = {}) {
  ...
  const jevConfig = apiKey !== undefined ? { apiKey, model: process.env.TYPESAFE_MODEL || 'jev-preview' } : await config();
