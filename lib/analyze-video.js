...
function cacheKey(body, maxComments) {
  ...
  return videoId ? `${SENTIMENT_VERSION}:localized:${videoId}:${body.analysisLanguage === 'en' ? 'en' : 'ko'}:${maxComments}` : null;
}
...
function writeCache(key, result) {
  if (!key || result.truncated || result.sentimentEngine !== 'jev') return;
...
export async function analyzeVideo(body, { maxAllowed = 200000, report = () => {}, allowPublicFallback = true, collectorUrl = '', apiKey } = {}) {
  ...
  // Visitor requests never share cached/inflight jobs or retain credentials in maps.
  const key = apiKey ? null : cacheKey(body, maxComments);
  ...
  const operation = runAnalysis(body, { maxComments, analysisLanguage, report, allowPublicFallback, collectorUrl, requestStartedAt, apiKey });
...
async function runAnalysis(body, { maxComments, analysisLanguage, report, allowPublicFallback, collectorUrl, requestStartedAt, apiKey }) {
  ...
  try {
    ...
    if (error.code !== 'JEV_NOT_CONFIGURED') {
      ...
      throw new Error('Jev 분석을 완료하지 못했습니다. 잘못된 중립 결과로 대체하지 않았습니다. 잠시 후 다시 시도해주세요.', { cause: error });
    }
