...
export async function analyzeVideo(body, { maxAllowed = DEFAULT_MAX_COMMENTS, report = () => {}, allowPublicFallback = true, collectorUrl = '', apiKey } = {}) {
  ...
  const cacheId = privateCache.key(apiKey, [extractVideoId(body.url), body.demo === true, analysisLanguage, maxComments, collectorUrl, allowPublicFallback, SENTIMENT_VERSION, 'opinions-v2', process.env.TYPESAFE_MODEL || 'jev-preview']);
...
async function runAnalysis(body, { maxComments, analysisLanguage, report, allowPublicFallback, collectorUrl, requestStartedAt, apiKey }) {
  ...
  report({ stage: 'collecting', done: 0, total: 0, percent: 5, videoContextStatus: videoContext.status === 'available' ? (videoContext.partial ? 'partial' : 'available') : 'unavailable' });
  ...
  const result = {
    video: source.video,
    videoContextStatus: videoContext.status === 'available' ? (videoContext.partial ? 'partial' : 'available') : 'unavailable',
