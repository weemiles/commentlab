...
// Every job carries its own key and is never shared. There is deliberately no
// result cache or in-flight deduplication: one visitor's provider call must not
// answer another visitor's request.
export async function analyzeVideo(body, { maxAllowed = DEFAULT_MAX_COMMENTS, report = () => {}, allowPublicFallback = true, collectorUrl = '', apiKey } = {}) {
  ...
  return runAnalysis(body, { maxComments, analysisLanguage, report, allowPublicFallback, collectorUrl, requestStartedAt, apiKey });
}
