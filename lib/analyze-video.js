...
import { fetchLocalTranscript } from './local-transcript.js';
...
export async function analyzeVideo(body, { maxAllowed = DEFAULT_MAX_COMMENTS, report = () => {}, allowPublicFallback = true, collectorUrl = '', apiKey } = {}) {
  ...
  const cacheId = privateCache.key(apiKey, [extractVideoId(body.url), body.demo === true, analysisLanguage, maxComments, collectorUrl, allowPublicFallback, SENTIMENT_VERSION, 'opinions-v3', process.env.TYPESAFE_MODEL || 'jev-preview']);
  const cached = privateCache.get(cacheId);
  if (cached) {
    report({ stage: 'complete', done: cached.summary.total, total: cached.summary.total, percent: 100, videoContextStatus: cached.videoContextStatus });
...
async function runAnalysis(body, { maxComments, analysisLanguage, report, allowPublicFallback, collectorUrl, requestStartedAt, apiKey }) {
  let videoContext = { status: 'unavailable', text: '' };
  if (body.demo !== true) {
    const videoId = extractVideoId(body.url);
    if (!videoId) throw localeError('invalid_url', { status: 400 });
    report({ stage: 'video_context', done: 0, total: 0, percent: 2 });
    videoContext = await fetchVideoTranscript({ videoId, language: analysisLanguage, collectorUrl });
    if (videoContext.status !== 'available' && allowPublicFallback && !collectorUrl) {
      videoContext = await fetchLocalTranscript({ videoId, language: analysisLanguage });
    }
  }
  report({ stage: 'collecting', done: 0, total: 0, percent: 5, videoContextStatus: videoContext.status === 'available' ? ((videoContext.partial || videoContext.text.length > 24000) ? 'partial' : 'available') : 'unavailable' });
  ...
  const result = {
    video: source.video,
    videoContextStatus: videoContext.status === 'available' ? ((videoContext.partial || videoContext.text.length > 24000) ? 'partial' : 'available') : 'unavailable',
