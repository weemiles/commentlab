...
export function buildReplyTargets(comments) {
  ...
  for (const c of comments) {
    ...
    result.set(c.id, { target: eligible.length === 1 ? eligible[0] : match?.candidate || null, evidence: match?.matches.filter(w=>w.length>2).slice(0,3) || [], candidates: eligible,
      status: eligible.length === 1 ? 'tagged' : match ? 'content' : eligible.length ? 'ambiguous' : 'missing' });
  }
  return result;
}
