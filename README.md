...
Before collecting comments, analysis attempts to read public YouTube captions (including automatic captions). The interface shows “영상 내용 분석 중” / “Analyzing video content”; transcript text stays out of progress events and analysis results. Jev receives the available transcript as untrusted context for interpreting references, quotations and targets, not as proof or an instruction. Missing or blocked captions do not stop comment analysis. This does not transcribe audio when captions are absent. Transcripts are retained internally up to 1,000,000 characters. For long videos, each classification batch receives up to 24,000 characters combining excerpts distributed across the video and passages matching the comments and parent context. These excerpts are explicitly marked partial; lexical retrieval can still miss indirect references. The remote collector must support the `transcript` action; older collectors safely fall back to comment-only context.

Local installations additionally try the already-installed `yt-dlp` when direct caption retrieval fails. No package is installed and no audio/video is downloaded. Available captions are retained even if a second language fails; temporary caption files are deleted after reading. The UI keeps a short caption availability status after the initial video-content stage, without exposing the transcript. Opinion grouping normalizes common Korean particles and merges overlapping topic groups before ranking, while keeping sentiment labels, explicit support/opposition, and negated or ambiguous stances separate. These are lexical groups, not a guarantee of semantic equivalence.
